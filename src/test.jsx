...
class ListDemo extends React.Component {
  ...
  render() {
    const liChildren = this.state.lastMsgs.map((msg) => {
      // 得到目标用户的id
      const targetUserId = msg.to===user._id ? msg.from : msg.to
      // 得到目标用户的信息
      const targetUser = users[targetUserId]
      const icon = targetUser.header ? require(`../../assets/images/${targetUser.header}.png`)
      ...
      >
        ...
        >
          <div className={`${this.props.className}-img`}>
            <img src={icon} width="44" height="44" onDragStart={e => e.preventDefault()} />
          </div>
